...
const User = () => {
	...
	const { authApi, user, setUser, openLogin, loadingLogin, loggedIn } =
		useGlobals()
	useEffect(() => {
		if (user && user.data) {
			authApi
				.get("/menuOptions/orderHistory")
				.then((response) => {
			...
			authApi
				.get("/reservation/reservationHistory")
				.then((response) => {
					setReservationHistoryData(response.data.Reservations)
				})
				.catch((error) =>
					console.log("error in getting reservation history: ", error)
				)
		} else {
	...
	const editProfile = (e) => {
		...
		if (
			...
			authApi
				.post("/users/editProfile", { formData, changePassword })
				.then((response) => {
